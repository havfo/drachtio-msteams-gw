...
class RtpEngine extends EventEmitter
{
	...
	{
		logger.debug('offer() [callId:"%s", rtpoptions:"%s"]', callId, JSON.stringify(rtpoptions));
		...
		const [ error, offerSdp ] = await safeRun(this._rtpengine.offer(
			this._rtpEngineConfig,
			Object.assign(
				{
					'sdp'      : sdp,
	...
	{
		logger.debug('answer() [callId:"%s", rtpoptions:"%s"]', callId, JSON.stringify(rtpoptions));
		...
		const [ error, answerSdp ] = await safeRun(this._rtpengine.answer(
			this._rtpEngineConfig,
			Object.assign(
				{
					'sdp'      : sdp,
	...
	async delete({ callId, fromTag })
	{
		...
		return await safeRun(this._rtpengine.delete(
			this._rtpEngineConfig,
			{
				'call-id'  : callId,
				'from-tag' : fromTag
